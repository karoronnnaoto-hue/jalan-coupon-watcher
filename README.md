...
既定値は「5,000円以上」**または**「実質割引率30%以上」です。50%以上はDiscord上で赤色の「高割引率」として表示します。
...
短時間の確認で先頭1ページだけ取得する場合は `--pages 1` を追加できます。
...
| 項目 | 既定値 | 意味 |
|---|---:|---|
| `minimum_coupon_yen` | `5000` | 通知する最低クーポン額 |
| `minimum_discount_rate` | `0.3` | 通知する最低実質割引率（30%） |
| `anomaly_discount_rate` | `0.5` | 高割引率として強調する基準（50%） |
| `match_mode` | `any` | `any` は金額・率の片方、`all` は両方を満たす場合に通知 |
| `pages_to_scan` | `3` | 先頭から監視するページ数（1ページ30件、最大34） |
...
監視頻度は5〜10分程度を推奨します。短すぎる間隔や全34ページの頻繁な取得は避けてください。
